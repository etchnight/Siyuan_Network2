...
class SiyuanConnect {
		...
		return new Promise((resolve, reject) => {
			...
			xhr.addEventListener('load', () => {
				try {
					...
					if (!result.code) {//个别api不返回code和msg，如获取文件
						resolve(response);
					}
					if (result.code != 0) {
						reject(result.msg);
	...
	putFile = async function (obj, path) {
		const file = new File([JSON.stringify(obj, null, 2)], obj.ID + ".sy", { type: 'application/json' })
		var formData = new FormData();
	...
	sql_FindbyID = async function (id) {
		const response = await this.sql("SELECT * FROM blocks WHERE id='" + id + "'");
		if (!response[0]) {
	...
	sql_FindbyParentID = async function (id) {
		const response = await this.sql("SELECT * FROM blocks WHERE parent_id='" + id + "'");
		return response;
	}
	//sql查询--查询父块
	sql_FindParentbyBlock = async function (block) {
		...
		if (block.type == "d") {//文档查父级文档
			let pathList = block.path.split("/")
			for (let i = 0; i < pathList.length; i++) {
				if (pathList[i].indexOf(block.id) == 0
					&& i >0
					&& pathList[i - 1]) {
	...
	sql_FindDefbyID = async function (id) {
		const response = await this.sql("SELECT * FROM blocks WHERE id IN"
	...
	sql_FindSpansbyTag = async function (tag) {
		const response = await this.sql("SELECT * FROM spans WHERE content='" + tag + "' AND type='tag'");
		return response;
	}

	//sql查询--根据type查询span表
	sql_FindSpansByType = async function (type) {
		const response = await this.sql("SELECT * FROM spans WHERE type='" + type + "'");
		return response;
	}

	//sql查询--根据url查询锚文本
	sql_FindAnchorByUrl = async function (url) {
		const spans = await this.sql("SELECT * FROM spans WHERE type='link_dest' AND " +
			"content='" + url + "'");
		var result = [];
		for (const span of spans) {
			const response = await this.sql("SELECT * FROM spans WHERE type='link_text' AND " +
	...
	sql_FindBlocksByKeyValue = async function (key, value) {
		key = "custom-" + key
		const response = await this.sql("SELECT * FROM blocks WHERE ial LIKE '%" + key
			+ '="' + value + "%'");
		for (var block of response) {
			const ial = jsonparse2(block.ial);
			block.ial = ial;
		}
		return response;
	}
	...
	jsonparse2(str) {
		...
		for (let i = 1; i < strList.length; i++) {
			...
			switch (lastItem) {
				case ":":
					if (item != "{" && item != "[" && punctuation.indexOf(item) != -1) {
						break;
					}
				case "}":
					if (item != "]" && item != "}" && punctuation.indexOf(item) != -1) {
		...
		for (let i = 1; i < strList.length; i++) {
			let item = strList[i];
			if (punctuation.indexOf(item) == -1 && !item.startsWith("'")
				&& !item.startsWith('"')) {
				strList[i] = '"' + item + '"';
